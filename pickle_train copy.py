...
class MixerModel(nn.Module):
    def __init__(
        ...
        self.inten_linear = nn.Linear(d_model, vocab_size, device=device)
        self.softplus = nn.Softplus()
        self.inner_linear = nn.Linear(d_model+d_time, d_model, device=device)
        self.eps = torch.finfo(torch.float32).eps
        self.d_time = 16
        self.norm = nn.LayerNorm(d_model, device="cuda:0")
    ...
    def forward(self, type_seq, time_seq, temp_seq = [], inference_params=None):
        ...
        init_cur_layer_ = torch.zeros_like(hidden_states).to(
                "cuda")
        ...
        for layer in self.layers:
            ...
            _temp_seq = torch.cat((cur_layer_, temp_seq.cuda()), dim=-1)
            ...
            cur_layer_ = self.inner_linear(hidden_states)[:, seqlen:, :]
            if layer == self.layers[-1]:
                break
            hidden_states = self.inner_linear(hidden_states)
            cur_layer_ = torch.tanh(cur_layer_) + cur_layer_
            
            # cur_layer_ = hidden_states[:, seqlen:, :]
...
class JsonlTestDataset(Dataset):
    def __init__(self, file_path):
        self.data = []
        self.lowest_age = 18
        self.highest_age = 91
...
if __name__ == '__main__':
    ...
    dataset = JsonlDataset('train.pkl')
    ...
    table = []
    eval_table = []
    test_dataset = JsonlTestDataset('test.pkl')
    train_dataloader = DataLoader(dataset, 1, shuffle=True, pin_memory=True, collate_fn=None)
    ...
    for epoch_num in range(epoch):
        ...
        for batchidx, sample in tqdm(enumerate(train_dataloader)):
            ...
            event_ll, non_event_ll, enc_inten = model.compute_loglik(time_seq=time_seq, event_seq=event_seq)
